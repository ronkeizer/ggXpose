...
# plot all layers
pl <- pl +
    geom_rect(data=vpc_dat, aes(xmin=lower, xmax=upper, ymin=vpc_dat[["X95.CI.for.5.from"]], ymax=vpc_dat[["X95.CI.for.5.to"]]), linetype=0, alpha=0.2, fill="#002288") +
    geom_rect(data=vpc_dat, aes(xmin=lower, xmax=upper, ymin=vpc_dat[["X95.CI.for.95.from"]], ymax=vpc_dat[["X95.CI.for.95.to"]]), linetype=0, alpha=0.2, fill="#002288") +
    geom_rect(data=vpc_dat, aes(xmin=lower, xmax=upper, ymin=vpc_dat[["X95.CI.for.50.from"]], ymax=vpc_dat[["X95.CI.for.50.to"]]), linetype=0, alpha=0.3, fill="#882222") +
